...
pub fn get_canister(input: TokenStream) -> TokenStream {
    ...
    let lit = match tokens.as_slice() { [TokenTree::Literal(lit)] => lit.to_string(), _ => panic!("Invalid input"), };
    let canister_name = &lit[1..lit.len() - 1];
    let mut vars = std::env::vars_os();
    let network_pair_opt = vars.find(|(key, _)| key.to_owned().into_string() == Ok("DFX_NETWORK".to_owned()));
    let network_opt = network_pair_opt.map(|(_, n)|n);

    if network_opt == Some(OsString::from("ic")) {
